...
export class TableController {
    ...
    renderizar(grupos, onFilaSeleccionada) {
        ...
        keys.forEach((key, index) => {
            ...
            // Combinar sectores
            const sectores = grupo.operaciones.map(op => op.sector || 'Sector no especificado');
            ...
            if (checkbox) {
                ...
                checkbox.addEventListener('change', (e) => {
                    ...
                    console.log('☑️ [DIAGNÓSTICO] filasMarcadas ahora:', Array.from(this.filasMarcadas));
                    this._notificarSeleccionCambiada();
                });
            }
    ...
    /**
     * ✅ Marca/desmarca todas las filas visibles. Pensado para conectar
     * a un checkbox "seleccionar todo" en el encabezado de la tabla.
    ...
    _calcularHorariosConsolidados(operaciones) {
        ...
        const horasInicioValidas = operaciones
            ...
            .filter(h => h && h !== '-' && h !== '' && h !== 'null' && h !== 'undefined');
        ...
        const tieneFinOperaciones = operaciones.some(op => {
            const hf = String(op.horaFinal || '').toUpperCase().trim();
        ...
        if (horasFinalValidas.length > 0) {
            ...
            operaciones.forEach(op => {
                ...
                let hiFormateada = hi.includes(':') ? hi : convertirHoraMilitar(hi) || hi;
                let hfFormateada = hf.includes(':') ? hf : convertirHoraMilitar(hf) || hf;
                ...
                try {
                    ...
                    if (isNaN(hiHoras) || isNaN(hiMinutos) || isNaN(hfHoras) || isNaN(hfMinutos)) return;
